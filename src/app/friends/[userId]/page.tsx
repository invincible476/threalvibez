      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {profileUser.instagramUrl && (
            <div className="pt-1">
              <a
                href={profileUser.instagramUrl.startsWith('http') ? profileUser.instagramUrl : `https://instagram.com/${profileUser.instagramUrl.replace('@', '').trim()}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-400 hover:text-violet-300 hover:bg-violet-500/20 transition-all text-sm font-medium"
              >
                ...
                <span className="text-xs text-violet-300">
                  @{profileUser.instagramUrl.match(/instagram\.com\/([^/]+)\/?$/)?.[1] || profileUser.instagramUrl.replace('@', '').trim()}
                </span>
              </a>
            </div>
          )}
